...
- Introduced concept of BFA file(literally BFlatA Arguments file) instead of RSP file, which can be used with -inc:<BFA file> option to combine multiple args profile to generate build script, and it supports macros and don't have to list args as "one arg per line".BFA file can be treated like a project file, which can be used to switch building different projects more easily, or with differnt sets of args.
